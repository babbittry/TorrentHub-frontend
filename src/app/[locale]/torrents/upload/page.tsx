...
export default function TorrentUploadPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.torrentFile) {
      newErrors.torrentFile = t('error_no_file');
    }

    if (!formData.description.trim()) {
      newErrors.description = t('error_description_required');
    } else if (formData.description.length > 4096) {
      newErrors.description = `${t('error_description_required')} (Max 4096 chars)`;
    }
  ...
  const handleFileSelect = (file: File) => {
    if (!file.name.endsWith('.torrent')) {
      setErrors({ torrentFile: t('error_invalid_file') });
      return;
    }

    if (file.size > 100 * 1024 * 1024) {
      setErrors({ torrentFile: t('error_file_too_large') });
      return;
    }
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!formData.torrentFile) {
      setErrorMessage(t('error_no_file'));
      return;
    }
          ...
          <form onSubmit={handleUpload}>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium mb-2">
                {t('selectFile')} <span className="text-danger">*</span>
              </label>
              <div
              ...
              >
                ...
                {formData.torrentFile ? (
                  <div>
                    <p className="text-success font-medium">{t('fileSelected')}</p>
            ...
            {torrentInfo && (
              <div className="mb-6 p-4 bg-default-100 rounded-lg">
                <h3 className="font-medium mb-2">{t('torrentInfo')}</h3>
            ...
            <div className="mb-6">
              <CustomInput
                label={t('imdb_label')}
                placeholder={t('imdb_placeholder')}
            ...
            {isUploading && (
              <div className="mb-6">
                <Progress
                  ...
                  label={`${t('uploadingProgress')}: ${uploadProgress}%`}
                  className="max-w-md"
                />
              </div>
            )}
